...
from cli.adapter_factory import (  # noqa: E402 -- path manipulation required
    create_worker,
)
from cli.command_history import (  # noqa: E402 -- path manipulation required
    CommandHistory,
)
...
from core.handlers import (  # noqa: E402 -- path manipulation required
    register_default_handlers,
)
from core.input_sanitiser import (  # noqa: E402 -- path manipulation required
    InputSanitiser,
)
...
from system.worker_persistence import (  # noqa: E402 -- path manipulation required
    WorkerPersistence,
)
...
class SelectionScreen(ModalScreen):
    ...
    def __init__(
        self, title: str, options: List[str], callback: Callable[[str], None]
    ) -> None:
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Container(Button("Cancel", id="cancel-btn"), id="selection-buttons"),
...
class CommandMenu(ListView):
    ...
    def _build_menu(self) -> None:
        ...
        for category, items in categories.items():
            ...
            self.append(
                ListItem(
                    Label(f"[bold]{category.upper()}[/bold]", id=f"cat-{category}")
                )
            )
...
class JarvisTUI(App):
    ...
    def __init__(self, emitter: TraceEmitter | None = None) -> None:
        ...
        if db_dsn:
            ...
            self.memory_router = MemoryRouter(
                postgres_backend=PostgresBackend(dsn=db_dsn, table_name="workers"),
        ...
        approval_trust = ApprovalTrustRegistry(
            memory_router=self.memory_router, emitter=self.emitter
        )
        ...
        # Create WorkerCircuitBreaker (prompt-78 S5 remediation)
        ...
        # Create ModelTierRouter (Plan 79)
        ...
        self.orchestrator = Orchestrator(
            ...
            emitter=self.emitter,
            worker_circuit_breaker=worker_circuit_breaker,  # prompt-78 S5 remediation
            degraded_mode_threshold=0.2,  # prompt-78 Plan (PEMADS Phase 2: 3-5 expert panel, any single failure → degraded)
            model_tier_router=model_tier_router,  # Plan 79
        )
        ...
        self.worker = create_worker(
            "ollama", "llama3", memory_router=self.memory_router
        )
        ...
        register_default_handlers(
            self.orchestrator, self.session_manager, input_sanitiser
        )
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Header()
        yield Container(
            Container(CommandMenu(id="menu"), id="menu-container"),
            Container(
                OutputDisplay(
                    "Welcome to Jarvis AI Agent Framework\n\nUse arrow keys to navigate menu\nPress Enter to select\nType commands directly below",
                    id="output",
                ),
                id="output-container",
            ),
    ...
    def _on_adapter_selected(self, adapter_name: str) -> None:
        ...
        try:
            ...
            self.worker = create_worker(
                adapter_name, "llama3", memory_router=self.memory_router
            )
