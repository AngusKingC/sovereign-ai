...
class CalendarSkill:
    ...
    async def get_upcoming(self, days: int = 7) -> list[dict]:
        ...
        try:
            ...
            ics_content = await loop.run_in_executor(
                None, lambda: open(calendar_path, "rb").read()
            )

            # Parse ICS
            calendar = await loop.run_in_executor(
                None, lambda: Calendar.from_ical(ics_content)
            )
    ...
    async def create_event(
        ...
        if self._approval_gate is None:
            raise SkillExecutionError(
                "Approval gate not injected — writing without approval is not permitted"
            )
        ...
        try:
            ...
            await loop.run_in_executor(
                None, lambda: open(calendar_path, "wb").write(ics_content)
            )
    ...
    async def cancel_event(self, uid: str) -> bool:
        ...
        if self._approval_gate is None:
            raise SkillExecutionError(
                "Approval gate not injected — modifying without approval is not permitted"
            )
