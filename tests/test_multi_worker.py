...
class TestMultiWorkerDispatcher:
    ...
    async def test_dispatch_queries_orchestrator_when_worker_ids_none(self) -> None:
        ...
        mock_orchestrator.get_top_candidates = AsyncMock(return_value=["worker_1", "worker_2"])
    ...
    async def test_dispatch_uses_provided_worker_ids(self) -> None:
        ...
        await dispatcher.dispatch("Test task", worker_ids=["worker_1", "worker_2"], mode="parallel")
    ...
    async def test_dispatch_drops_workers_that_fail_budget_check(self) -> None:
        ...
        mock_resource_budget.check_all_budgets = AsyncMock(side_effect=lambda wid: wid == "worker_1")
        ...
        result = await dispatcher.dispatch("Test task", worker_ids=["worker_1", "worker_2"], mode="parallel")
    ...
    async def test_parallel_all_workers_receive_original_task(self) -> None:
        ...
        task = "Original task"
        await dispatcher.dispatch(task, worker_ids=["worker_1", "worker_2"], mode="parallel")
    ...
    async def test_parallel_returns_multi_worker_result_with_correct_fields(self) -> None:
    ...
    async def test_sequential_each_worker_receives_original_task(self) -> None:
        ...
        task = "Original task"
        await dispatcher.dispatch(task, worker_ids=["worker_1", "worker_2"], mode="sequential")
    ...
    async def test_sequential_ensure_model_called_before_each_worker(self) -> None:
        ...
        mock_resource_manager.ensure_model = AsyncMock()
        mock_resource_manager.release_model = AsyncMock()
        ...
        # ensure_model should be called before each worker
        assert mock_resource_manager.ensure_model.call_count == 2
        mock_resource_manager.ensure_model.assert_any_call(mock_worker_1.adapter)
        mock_resource_manager.ensure_model.assert_any_call(mock_worker_2.adapter)
    ...
    async def test_sequential_release_model_called_after_each_worker(self) -> None:
        ...
        # release_model should be called after each worker (2 times) + orchestrator (1 time) = 3 total
        assert mock_resource_manager.release_model.call_count == 3
    ...
    async def test_sequential_failed_worker_continues_chain(self) -> None:
        ...
        result = await dispatcher.dispatch("Test task", worker_ids=["worker_1", "worker_2"], mode="sequential")
    ...
    async def test_select_winner_calls_rating_system_for_winner_and_non_winners(self) -> None:
    ...
    async def test_orchestrator_release_model_called_before_any_worker(self) -> None:
        ...
        # Test parallel mode - orchestrator release only (worker doesn't release in parallel)
        await dispatcher.dispatch("Test task", worker_ids=["worker_1"], mode="parallel")
        assert mock_resource_manager.release_model.call_count == 1
        
        # Reset mock
        mock_resource_manager.reset_mock()
        
        # Test sequential mode - orchestrator release + worker release
        await dispatcher.dispatch("Test task", worker_ids=["worker_1"], mode="sequential")
        assert mock_resource_manager.release_model.call_count == 2
    ...
    async def test_release_failure_does_not_abort_dispatch(self) -> None:
        ...
        mock_resource_manager = MagicMock()
        mock_resource_manager.release_model = AsyncMock(side_effect=Exception("Release failed"))
