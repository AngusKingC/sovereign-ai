...
Integration tests require GROQ_API_KEY environment variable.
"""
...
@pytest.fixture
def mock_adapter(mock_groq_client):
    """Groq adapter with mocked client — for unit tests."""
    adapter = GroqAdapter(api_key="mock-key", model_name="llama-3.3-70b-versatile")
    # Force eager client creation so test_close has something to close
    # (GroqAdapter.__init__ sets _client=None lazily; close() does
...
class TestGroqAdapterUnit:
    """Unit tests for GroqAdapter — mocked, no API key required."""

    def test_initialization(self, mock_adapter):
        """Test adapter initialization."""
        assert mock_adapter.model_name == "llama-3.3-70b-versatile"
        assert mock_adapter.is_local is False
        assert mock_adapter.cost_per_token == 0.00059
...
@pytest.fixture
def adapter(api_key):
    """Create Groq adapter with real API key — for integration tests."""
    return GroqAdapter(api_key=api_key, model_name="llama-3.3-70b-versatile")


class TestGroqAdapterIntegration:
    """Integration tests for GroqAdapter — require real GROQ_API_KEY."""
