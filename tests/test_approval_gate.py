...
class TestApprovalGate:
    ...
    async def test_respond_with_approved_false_transitions_to_denied_and_raises_error(
        self,
    ) -> None:
    ...
    async def test_respond_with_unknown_request_id_raises_worker_not_found_error(
        self,
    ) -> None:
    ...
    async def test_check_scope_returns_true_when_matching_scope_exists(self) -> None:
        ...
        result = await gate.check_scope(
            "sess_xyz", "file_write", {"path": "/tmp/test.txt"}
    ...
    async def test_load_scopes_initializes_cache_for_session(self) -> None:
        """Test load_scopes initializes cache for session and calls Postgres query."""
        mock_router = MockMemoryRouter()
        mock_state_machine = MockStateMachine()
        emitter = MemoryTraceEmitter()

        # Mock postgres_trace_store.fetch to return test data
        mock_postgres_store = MagicMock()
        mock_postgres_store.fetch = AsyncMock(
            return_value=[
                {
                    "scope_id": "scope_123",
                    "session_id": "sess_xyz",
        ...
        mock_router.postgres_trace_store = mock_postgres_store
        ...
        # Verify Postgres query was called
    ...
    async def test_approval_gate_skips_gate_when_command_is_trusted(self) -> None:
        ...
        trust_registry = ApprovalTrustRegistry(
            memory_router=None, emitter=trust_emitter
        )
        await trust_registry.set_trust(
            "git status", TrustLevel.PERMANENT_TRUST, scope="permanent"
        )

        # Create gate with trust registry
        gate = ApprovalGate(
            mock_state_machine, mock_router, emitter, trust_registry=trust_registry
        )
