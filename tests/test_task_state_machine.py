...
class TestTaskStateMachine:
    ...
    async def test_all_invalid_transitions_raise_invalid_state_transition_error(
        self,
    ) -> None:
    ...
    async def test_can_transition_returns_correct_bool_without_mutating_task(
        self,
    ) -> None:
    ...
    async def test_is_terminal_returns_true_for_complete_failed_cancelled_denied(
        self,
    ) -> None:
    ...
    async def test_get_valid_next_states_returns_correct_list_for_each_state(
        self,
    ) -> None:
        ...
        test_cases = [
            ...
            (
                TaskStatus.PLANNED,
                [TaskStatus.EXECUTING, TaskStatus.FAILED, TaskStatus.CANCELLED],
            ),
            (
                TaskStatus.EXECUTING,
                [
    ...
    async def test_state_history_is_appended_on_each_transition(self) -> None:
        ...
        task = await state_machine.transition(
            task, TaskStatus.PLANNED, reason="Planning", actor="orchestrator"
        )
        task = await state_machine.transition(
            task, TaskStatus.EXECUTING, reason="Executing", actor="worker"
        )
        task = await state_machine.transition(
            task, TaskStatus.VALIDATING, reason="Validating", actor="orchestrator"
        )
    ...
    async def test_trace_events_emitted_on_each_transition(self) -> None:
        ...
        # Perform transition
        task = await state_machine.transition(
            task, TaskStatus.PLANNED, reason="Test", actor="test"
        )
        ...
        assert (
            mock_router.writes[0]["metadata"]["from_state"] == TaskStatus.RECEIVED.value
        )
        assert mock_router.writes[0]["metadata"]["to_state"] == TaskStatus.PLANNED.value

    async def test_invalid_state_transition_error_contains_correct_from_state_to_state_task_id(
        self,
    ) -> None:
    ...
    async def test_backward_compatibility_existing_status_field_alias_still_works(
        self,
    ) -> None:
    ...
    async def test_task_with_awaiting_approval_state_is_held_in_orchestrator_pending_queue(
        self,
    ) -> None:
        """Test that task with AWAITING_APPROVAL state is held in orchestrator pending queue."""
        from core.orchestrator import Orchestrator
        from core.schemas import WorkerOutput, WorkerProfile
        from core.worker_base import WorkerBase

        mock_router = MockMemoryRouter()
        orchestrator = Orchestrator(
            mock_router, worker_circuit_breaker=None, degraded_mode_threshold=0.5
        )
        ...
        assert (
            orchestrator.pending_approval_queue[0].current_state
            == TaskStatus.AWAITING_APPROVAL
        )
    ...
    async def test_orchestrator_transitions_task_through_full_happy_path(self) -> None:
        ...
        assert (
            len(task.state_history) >= 4
        )  # PLANNED -> EXECUTING -> VALIDATING -> COMPLETE
        ...
        assert expected_states.issubset(
            actual_states
        ), f"Expected {expected_states} to be subset of {actual_states}"
    ...
    async def test_orchestrator_transitions_to_failed_on_worker_error(self) -> None:
        ...
        failed_transition = [
            t for t in task.state_history if t.to_state == TaskStatus.FAILED
        ]
