"""
Tests for Together adapter.

Unit tests use mocks and run without API key.
Integration tests require TOGETHER_API_KEY environment variable.
"""
...
def mock_together_client():
    """Mock AsyncOpenAI client for Together unit tests.
    
    Together uses AsyncOpenAI with base_url="https://api.together.xyz/v1" —
    OpenAI-compatible response shape (choices[0].message.content).
    """
    with patch('adapters.together.AsyncOpenAI') as mock_class:
...
@pytest.fixture
def mock_adapter(mock_together_client):
    """TogetherAdapter with mocked client — for unit tests."""
    from adapters.together import TogetherAdapter
    adapter = TogetherAdapter(api_key="mock-key", model_name="mistralai/Mixtral-8x7B-Instruct-v0.1")
...
class TestTogetherAdapterUnit:
    """Unit tests — mocked, no API key required."""

    def test_initialization(self, mock_adapter):
        assert mock_adapter.model_name == "mistralai/Mixtral-8x7B-Instruct-v0.1"
...
class TestTogetherAdapterIntegration:
    """Integration tests — require real TOGETHER_API_KEY."""
    ...
    def adapter(self):
        from adapters.together import TogetherAdapter
        return TogetherAdapter(api_key=os.getenv("TOGETHER_API_KEY"), model_name="mistralai/Mixtral-8x7B-Instruct-v0.1")
