...
**Files Modified**:
- Adapters/gemini.py — Migrated from google.generativeai to google.genai SDK (removed # noqa suppression, updated imports, client initialization, async API calls)
- Tests/test_lm_studio_adapter.py — Fixed test_health_check_without_server (mocked HTTP client), added test_health_check_with_server (integration test)
- Tests/test_anthropic_adapter.py — Converted 5 unit tests to mocked (run always without API key), kept 7 integration tests env-conditional
- Tests/test_gemini_adapter.py — Converted 5 unit tests to mocked (run always without API key), kept 6 integration tests env-conditional
