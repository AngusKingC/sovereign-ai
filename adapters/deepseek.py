"""
DeepSeek cloud LLM adapter.

Single responsibility: Handle interactions with DeepSeek's API,
providing access to strong coding and general-purpose models.
"""
...
class DeepSeekAdapter(LLMAdapter):
    """DeepSeek adapter for strong coding and general-purpose model inference."""
    ...
    async def generate(
        ...
        Uses DeepSeek's OpenAI-compatible API for strong coding model inference.
        """
        ...
        try:
            ...
            try:
                ...
                await self._emitter.emit(TraceEvent(
                    ...
                    component=TraceComponent.ADAPTER,
                    message="Trace emission failed in DeepSeek adapter call start",
            ...
            try:
                ...
                await self._emitter.emit(TraceEvent(
                    event_type=TraceEventType.ADAPTER_ERROR,
                    component=TraceComponent.ADAPTER,
                    message="Trace emission failed in DeepSeek adapter response",
                    level=TraceLevel.WARNING,
                    data={"error": str(e)},
                    duration_ms=0,
                ))
            ...
            try:
                ...
                await self._emitter.emit(TraceEvent(
                    event_type=TraceEventType.ADAPTER_ERROR,
                    component=TraceComponent.ADAPTER,
                    message="Trace emission failed in DeepSeek adapter error handler",
                    level=TraceLevel.WARNING,
    ...
    def cost_per_token(self) -> float:
        """Cost per token for this model (DeepSeek: ~$0.001/1K tokens)."""
        costs = {
            "deepseek-chat": 0.001,
            "deepseek-coder": 0.001,
        }
        return costs.get(self._model_name, 0.001)
