...
class ApprovalRequest(BaseModel):
    ...
    # Action details
    action_type: ApprovalActionType = Field(
    ...
    timeout_seconds: int = Field(
        default=300, ge=30, le=3600, description="Timeout in seconds"
    )
    expires_at: datetime = Field(..., description="When request expires")

    # Status
    status: Literal["pending", "approved", "denied", "expired"] = Field(
        default="pending", description="Request status"
    )
    approved_by: Optional[str] = Field(
        default=None, description="Who approved (user ID)"
    )
    approved_at: Optional[datetime] = Field(
        default=None, description="When approval was granted"
    )
    denied_reason: Optional[str] = Field(default=None, description="Reason for denial")

    # Scope matching
    matched_scope_id: Optional[str] = Field(
        default=None, description="If auto-approved by scope"
    )
...
class ApprovalResponse(BaseModel):
    ...
    scope_id: Optional[str] = Field(
        default=None, description="If approved by scope, scope ID"
    )
...
class ApprovalScope(BaseModel):
    ...
    # Scope definition
    scope_type: Literal["file_write", "download", "network", "command"] = Field(
        ..., description="Type of scope"
    )
    scope_pattern: str = Field(..., description="Pattern matching rule")

    # Limits
    size_limit_mb: Optional[int] = Field(
        default=None, ge=0, description="Max size in MB"
    )
    time_limit_seconds: Optional[int] = Field(
        default=None, ge=0, description="Max duration in seconds"
    )

    # Timing
    granted_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="When scope was granted",
    )
    ...
    is_active: bool = Field(
        default=True, description="Whether scope is currently active"
    )
    revoked_at: Optional[datetime] = Field(
        default=None, description="When scope was revoked"
    )
    revoked_by: Optional[str] = Field(
        default=None, description="User who revoked the scope"
    )
...
class ApprovalGate:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "AR18: nested trace emit failed: %s",
                        nested_error,
                        exc_info=True,
                    )
    ...
    async def submit_for_approval(
        self, proposal_id: str, description: str, context: dict
    ) -> None:
        ...
        if approved:
            ...
            if always_approve and self.trust_registry is not None:
                ...
                if "command" in request.action_parameters:
                    command = request.action_parameters["command"]
                try:
                    await self.trust_registry.set_trust(
                        command, TrustLevel.PERMANENT_TRUST, scope="permanent"
                    )
                except Exception as trust_error:
