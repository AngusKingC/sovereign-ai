...
class Task(BaseModel):
    ...
    status: TaskStatus = Field(
        ...
        description="Current task status (use current_state for new code)",
    )
    current_state: TaskStatus = Field(
        default=TaskStatus.RECEIVED, description="Current task state"
    )
    state_history: list[TaskStateTransition] = Field(
...
class WorkerProfile(BaseModel):
    ...
    capabilities: list[str] = Field(
        ...
        description="List of capability keywords for worker routing",
    )
    preferred_complexity: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="Preferred task complexity score (0.0-1.0)",
    )

    @field_validator(
        "depth_preference",
        "speculation_tolerance",
        "source_skepticism",
        "verbosity",
...
class QuantisationVariant(BaseModel):
    ...
    ram_required_gb: float = Field(
        ge=0.0, description="RAM required in GB for CPU offload"
    )
    quality_score: float = Field(
        ge=0.0, le=1.0, description="Quality score (0.0-1.0, higher is better)"
    )
    speed_score: float = Field(
        ge=0.0, le=1.0, description="Speed score (0.0-1.0, higher is faster)"
    )
...
class ModelEntry(BaseModel):
    ...
    model_id: str = Field(
        description="Unique identifier (e.g. ollama/qwen2.5-coder:7b)"
    )
    name: str = Field(description="Human readable name")
    source: ModelSource = Field(description="Model source")
    adapter_compatibility: list[str] = Field(
        default_factory=list, description="Adapter names that can run this model"
    )
    task_tags: list[str] = Field(
        default_factory=list,
        description="Task suitability tags (e.g. code, reasoning, chat)",
    )
    quantisation_variants: list[QuantisationVariant] = Field(
        default_factory=list, description="Available quantisation variants"
    )
    download_status: DownloadStatus = Field(
        default=DownloadStatus.NOT_DOWNLOADED, description="Download status"
    )
    downloaded_quantisation: str | None = Field(
        default=None, description="Which variant is currently downloaded"
    )
...
class LoadedModel(BaseModel):
    ...
    is_pinned: bool = Field(
        default=False, description="Whether model is pinned (never auto-evicted)"
    )
    task_priority: TaskPriority = Field(
        default=TaskPriority.NORMAL, description="Task priority for eviction decisions"
    )
...
class LoadDecision(BaseModel):
    ...
    models_to_evict: list[str] = Field(
        default_factory=list, description="Model IDs to evict"
    )
...
from typing import (  # noqa: E402 -- conditional import for Protocol definition
    Protocol,
    runtime_checkable,
)
...
class DownloadResult(BaseModel):
    ...
    model_id: str = Field(description="Model identifier")
    quantisation: str | None = Field(
        default=None, description="Quantisation variant downloaded"
    )
...
class InstructionFile(BaseModel):
    ...
    version: int = Field(
        description="Instruction file version (starts at 1, increments on update)"
    )
...
class InstructionChangelogEntry(BaseModel):
    ...
    rating_trend: float | None = Field(
        default=None, description="Rating trend at time of update"
    )
...
class VersionUpdateProposal(BaseModel):
    ...
    proposal_type: str = Field(
        default="instruction_tweak",
        description="Type of proposal — determines AutoCorrector safe/unsafe classification. "
        "Values: 'instruction_tweak' (safe), 'routing_weight' (safe), "
        "'code_change' (unsafe), 'model_download' (unsafe). "
        "Defaults to 'instruction_tweak' for backward compatibility.",
    )
...
class OrchestratorMetrics(BaseModel):
    ...
    task_id: str = Field(description="Task identifier")
    routed_to_worker_id: str = Field(
        description="Worker that was selected for this task"
    )
    routing_score: float = Field(
        description="Score assigned to selected worker at routing time"
    )
    task_completed: bool = Field(
        description="Whether the task reached a terminal success state"
    )
    user_rating: float | None = Field(
        default=None, description="Manual rating if provided (1-10), else None"
    )
    timestamp: datetime = Field(
...
class EvaluatorScore(BaseModel):
    ...
    format_compliance: float = Field(
        description="Followed output format instructions (0.0-1.0)"
    )
...
class EvaluationRecord(BaseModel):
    ...
    record_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique record identifier"
    )
    task_id: str = Field(description="Task identifier")
    worker_id: str = Field(description="Worker identifier")
    evaluator_score: EvaluatorScore | None = Field(
        default=None, description="Auto-eval score if performed"
    )
    manual_rating: float | None = Field(
        default=None, description="Manual rating 1-10 if provided"
    )
...
class Scratchpad(BaseModel):
    ...
    scratchpad_id: UUID = Field(
        default_factory=uuid4, description="Unique scratchpad identifier"
    )
    task_id: UUID = Field(description="Task identifier")
    entries: list[ScratchpadEntry] = Field(
        default_factory=list, description="All entries in the scratchpad"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="When scratchpad was created"
    )
    completed_at: datetime | None = Field(
        default=None, description="When scratchpad was compacted"
    )
    summary: str | None = Field(
        default=None, description="Compacted summary of scratchpad"
    )
    is_compacted: bool = Field(
        default=False, description="Whether scratchpad has been compacted"
    )
