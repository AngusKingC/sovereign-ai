...
class MultiWorkerDispatcher:
    ...
    async def dispatch(
        self, task: str, worker_ids: list[str] | None = None, mode: str = "sequential"
    ) -> MultiWorkerResult:
        ...
        if mode not in ("parallel", "sequential"):
            raise ValueError(
                f"Invalid mode: {mode}. Must be 'parallel' or 'sequential'"
            )
        ...
        if worker_ids is None:
            worker_ids = await self.orchestrator.get_top_candidates(
                task, self.max_workers
            )
    ...
    async def _dispatch_parallel(
        self, task: str, worker_ids: list[str]
    ) -> list[WorkerResponse]:
