...
class RatingSystem:
    ...
    async def get_ratings(self, worker_id: str, limit: int = 50) -> list[WorkerRating]:
        ...
        results = await self.memory_router.fetch_by_filter(
            filter={"worker_id": worker_id}, collection="worker_ratings", limit=limit
        )
        ...
        for result in results:
            ...
            try:
                rating = WorkerRating(
                    ...
                    instruction_file_version=rating_data.get(
    ...
    async def get_average_score(
        self, worker_id: str, last_n: int | None = None
    ) -> float | None:
    ...
    async def get_trend(self, worker_id: str, window: int = 10) -> float | None:
    ...
    async def get_best_model(self, worker_id: str) -> str | None:
    ...
    async def record_comparison(
        self, task_id: str, winner_worker_id: str, loser_worker_id: str, model_used: str
    ) -> None:
    ...
    async def record_rating(
        ...
        if (
            ...
            and instruction_file_version is None
            and comment is None
        ):
