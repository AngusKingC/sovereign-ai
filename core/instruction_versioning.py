...
When an AutoCorrector instance is provided (optional, defaults to None), safe proposals
are auto-applied via AutoCorrector.apply_proposal() instead of going to ApprovalGate.
Unsafe proposals continue through the existing ApprovalGate path. When auto_corrector
...
from core.auto_corrector import ApplyStatus
...
class InstructionVersionManager:
    ...
    async def check_and_trigger_update(
        self, profile: DynamicWorkerProfile
        ...
        current_instruction = await self.instruction_generator.get_instruction_file(
            profile.worker_id
        )
        ...
        trigger_reason = (
            f"rating trend {rating_trend:.2f} over last {len(ratings)} tasks"
        )

        updated_instruction = await self.instruction_generator.update_instruction_file(
            profile=profile, existing=current_instruction, trigger=trigger_reason
        )
        ...
        # Submit to ApprovalGate — OR delegate to AutoCorrector if configured.
        # When auto_corrector is None (default), behavior is unchanged: every
        # proposal goes to ApprovalGate. When auto_corrector is set, it decides
        # whether to auto-apply (safe) or escalate (unsafe).
        if self.auto_corrector is not None:
            # AutoCorrector handles both apply and escalate paths.
            # If it returns APPLIED, the proposal is already approved via
            # self.approve_update() (called by AutoCorrector) — which also
            # updates the proposal status in memory_router AND clears
            # _pending_proposals (lines 244-246 of approve_update).
            # If it returns ESCALATED, AutoCorrector has already called
            # submit_for_approval with richer context.
            # If it returns ERROR, neither approve_update nor submit_for_approval
            # ran to completion — _pending_proposals is still populated. We MUST
            # pop it here, otherwise every future check_and_trigger_update for
            # this worker hits the collision guard (line 70) and returns early,
            # freezing the worker out of future improvements indefinitely.
            # (Rev2 fix for Claude Issue 2 — _pending_proposals leak on ERROR.)
            result = await self.auto_corrector.apply_proposal(proposal)
            if result.status == ApplyStatus.ERROR:
                self._pending_proposals.pop(proposal.worker_id, None)
        else:
            # Existing behavior — submit to ApprovalGate for human review.
    ...
    async def approve_update(self, proposal: VersionUpdateProposal) -> InstructionFile:
        ...
        if not current_instruction:
            raise ValueError(
                f"No current instruction file found for worker {proposal.worker_id}"
            )
    ...
    async def reject_update(self, proposal: VersionUpdateProposal) -> None:
    ...
    async def rollback(self, worker_id: str, target_version: int) -> InstructionFile:
        ...
        if not target_instruction:
            raise ValueError(
                f"Version {target_version} not found for worker {worker_id}"
            )
        ...
        if not current_instruction:
            raise ValueError(
                f"No current instruction file found for worker {worker_id}"
            )
    ...
    async def get_version_history(self, worker_id: str) -> list[InstructionFile]:
        ...
        results = await self.memory_router.fetch_by_filter(
            filter={"worker_id": worker_id}, collection="instruction_files", limit=1000
        )
