...
class TraceOptimiser:
    ...
    async def score_recent_traces(self, worker_id: str, n: int = 50) -> float:
        ...
        try:
            ...
            tool_call_events = [
                ...
                isinstance(e.get("event_type"), str) and
                ("tool_call" in e["event_type"] or "skill_call" in e["event_type"] or "mcp_tool_call" in e["event_type"])
            ]
            ...
            error_events = [
                e for e in trace_events
                if isinstance(e, dict) and
                isinstance(e.get("level"), str) and
                e["level"].lower() == "error"
            ]
    ...
    async def check_and_trigger_update(self, worker_id: str) -> VersionUpdateProposal | None:
        ...
        try:
            ...
            try:
                ...
                proposal = await self.instruction_version_manager.check_and_trigger_update(profile)
                ...
                try:
                    await self.emitter.emit(TraceEvent(
                        ...
                        data={"worker_id": worker_id, "score": score, "threshold": self.trace_threshold},
                        duration_ms=0
                    ))
                except Exception:
                    pass
